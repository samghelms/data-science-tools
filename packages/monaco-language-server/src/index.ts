...
function createDocument(model: editor.IReadOnlyModel) {
...
export const configureLanguageServer = (cellsManager: CellsManager) => {
    languages.registerCompletionItemProvider('markdown', {
        ...
        resolveCompletionItem(item: languages.CompletionItem, token: CancellationToken): PromiseLike<languages.CompletionItem> {
            ...
            const res = client.then(
                cl => cl.sendRequest(CompletionResolveRequest.type, code2ProtocolConverter.asCompletionItem(item), token).then(
