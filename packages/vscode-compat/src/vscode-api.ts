...
import { Disposable } from "monaco-languageclient/lib/disposable";
import {
...
export function createVSCodeApi(servicesProvider: Services.Provider, getCellsManager: any): typeof vscode {
    ...
    const languages: typeof vscode.languages = {
        ...
        createDiagnosticCollection(name?: string): vscode.DiagnosticCollection {
            ...
            return {
                ...
                set(arg0: vscode.Uri | [vscode.Uri, vscode.Diagnostic[] | undefined][], arg1?: vscode.Diagnostic[] | undefined): void {
                    if (collection) {
                        if (arg1) {
                            collection.set(arg0.toString(), (arg1 as any) as Diagnostic[]);
                        } else {
        ...
        registerCompletionItemProvider(selector, provider, ...triggerCharacters) {
            ...
            const cm = getCellsManager();
            cm.setLanguageServerProvider(provider, selector)
            const resolveCompletionItem = provider.resolveCompletionItem;
            const _selector = ["markdown"];
            return languages.registerCompletionItemProvider(_selector, {
                async provideCompletionItems({ textDocument, position, context }, token) {
                    // console.log(getCellsManager())
                    const cm = getCellsManager();
                    const cell = await cm.inCellValue(position.line)
                    const cellProvider = cell ? cell.getLanguageServerProvider() : null;
                    // console.log('providing completion')
                    if (cell && cellProvider) {
                        return cellProvider.provideCompletionItems(<any>textDocument, <any>position, token, context || {
